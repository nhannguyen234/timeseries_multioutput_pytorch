...
class GRUModel(nn.Module):
    ...
    def fit(self, xs, ys):
        ...
        y = torch.tensor(ys, dtype=float).transpose(1, 0)
        ...
        for i in range(self.epochs):
            ...
            for input, target in train_seq:
                ...
                self.optimizer.step()
                self.scheduler.step(single_loss)

            if i % 5 == 0:
                print("it. %i / %i - loss = %.8f" % (i, self.epochs, single_loss))
...
class MyModel(ModelApi):
    ...
    def load(cls, model_dir: str):
        ...
        with open(os.path.join(model_dir, 'model.pth'), 'rb') as f:
            my_model.nn_model.load_state_dict(torch.load(f, map_location=torch.device('cpu')))
